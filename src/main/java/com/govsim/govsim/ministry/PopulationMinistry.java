...
/**
 * Population Ministry implementation (Citizens)
 */
public class PopulationMinistry extends Ministry {
    ...
    public PopulationMinistry(President president) {
        super("Population");
    ...
    protected void handleNormal(Event event) {
        ...
        System.out.println("[Population] Handled normal event: " + event.getDescription());
    }

    @Override
    protected void handleDangerous(Event event) {
        escalated++;
        System.out.println("[Population] ESCALATING to President: " + event);
        if (president != null) {
    ...
    public Report generateReport(int month, int year) {
        ...
        double rating = 100.0 - (escalated * 12);
